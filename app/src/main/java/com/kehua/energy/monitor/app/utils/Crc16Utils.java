...
public class Crc16Utils {
	private static byte[] auchCRCHi = { (byte) 0x00, (byte) 0xC1, (byte) 0x81, (byte) 0x40, (byte) 0x01, (byte) 0xC0,
	...
	/**
	 * @param datas 仅包含数据的字节，不允许包含CRC标志位
	 * @return CRC16标志位，包含2个Byte
	 */
	public static byte[] crc16(byte[] datas) {
		int usDataLen = datas.length;
		byte uchCRCHi = (byte) 0xFF;
		byte uchCRCLo = (byte) 0xFF;

		int uIndex;
		for (int i = 0; i < usDataLen; i++) {
			uIndex = (uchCRCHi ^ datas[i]) & 0xff;
			uchCRCHi = (byte) (uchCRCLo ^ auchCRCHi[uIndex]);
			uchCRCLo = auchCRCLo[uIndex];
		}
		
		byte[] result = { uchCRCHi, uchCRCLo};
		return result;
	}
	
	
	/**
	 * @param datasEndOfCrc 字节数组最后2位为CRC标志
	 * @return 若最后2位crc标志位正确则返回true，否则返回false。
	 */
	public static boolean checkCrc16(byte[] datasEndOfCrc) {
		if (datasEndOfCrc.length <= 2)
			return false;

		byte[] datas = new byte[datasEndOfCrc.length - 2];
		System.arraycopy(datasEndOfCrc, 0, datas, 0, datas.length);

		byte[] crc = crc16(datas);
		boolean result =  crc[0] == datasEndOfCrc[datasEndOfCrc.length - 2] && crc[1] == datasEndOfCrc[datasEndOfCrc.length - 1];
		return result;
	}

	public static boolean checkCrc16(String hex) {
		return checkCrc16(ByteUtils.hexStringToBytes(hex));
	}
}
