...
public class HotspotListAdapter extends BaseQuickAdapter<HotspotInfo,BaseViewHolder>{
    ...
    protected void convert(BaseViewHolder helper, HotspotInfo item) {
        ...
        if(item.getLevel()>=-80){
            helper.setText(R.id.tv_level, Fastgo.getContext().getString(R.string.信号强));
        }else if(-80>item.getLevel() && item.getLevel()>=-110){
            helper.setText(R.id.tv_level,Fastgo.getContext().getString(R.string.信号一般));
        }else {
            helper.setText(R.id.tv_level,Fastgo.getContext().getString(R.string.信号弱));
        }
