...
public class ClassicsFooter extends InternalClassics<ClassicsFooter> implements RefreshFooter {
    ...
    public ClassicsFooter(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (REFRESH_FOOTER_PULLING == null) {
            REFRESH_FOOTER_PULLING = Fastgo.getContext().getString(R.string.srl_footer_pulling);
        }

        if (REFRESH_FOOTER_RELEASE == null) {
            REFRESH_FOOTER_RELEASE = Fastgo.getContext().getString(R.string.srl_footer_release);
        }

        if (REFRESH_FOOTER_LOADING == null) {
            REFRESH_FOOTER_LOADING = Fastgo.getContext().getString(R.string.srl_footer_loading);
        }

        if (REFRESH_FOOTER_REFRESHING == null) {
            REFRESH_FOOTER_REFRESHING = Fastgo.getContext().getString(R.string.srl_footer_refreshing);
        }

        if (REFRESH_FOOTER_FINISH == null) {
            REFRESH_FOOTER_FINISH = Fastgo.getContext().getString(R.string.srl_footer_finish);
        }

        if (REFRESH_FOOTER_FAILED == null) {
            REFRESH_FOOTER_FAILED = Fastgo.getContext().getString(R.string.srl_footer_failed);
        }

        if (REFRESH_FOOTER_NOTHING == null) {
            REFRESH_FOOTER_NOTHING = Fastgo.getContext().getString(R.string.srl_footer_nothing);
        }
